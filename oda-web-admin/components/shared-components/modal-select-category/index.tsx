  ...
  '& .MuiList-root': {
    '& .MuiListItem-root': {
      '&.active': {
        ...
        '& .MuiTypography-root': {
          fontWeight: '500',
          color: theme.palette.customColors.selectedListItemColor,
        },
      },
    },
  },
...
const FormSelectCategory = (props: IFormSelectCategory) => {
  ...
  const listCategoriesLevel1 = useMemo(() => {
    if (categoryLevel1?.success) {
      const categoriesProcess: ICategory[] = categoryLevel1?.data as any;
  ...
  const listCategoriesLevel2 = useMemo(() => {
    if (categoryLevel2?.success) {
      const categoriesProcess: ICategory[] = categoryLevel2?.data as any;
      if (!isEmpty(categoriesProcess)) {
        return categoriesProcess;
      }
      return [];
  ...
  const listCategoriesLevel3 = useMemo(() => {
    ...
    if (categoryLevel3?.success) {
      const categoriesProcess: ICategory[] = categoryLevel3?.data as any;
      if (!isEmpty(categoriesProcess)) {
        ...
        >
          {listCategory.map((item) => {
            return (
              <ListItem
                key={item?.id}
                className={selectCategory?.id === item?.id ? 'active' : ''}
                secondaryAction={
  ...
  const handleSubmit = () => {
    ...
    if (selectedCategories) {
      const result = selectedCategories.findIndex(
        (cat) => Number(cat.id) === idCategoryLast
      );
  ...
  const isDisableButtonApply = useMemo(() => {
    if (
      (isMaxLevel && maxLevel) ||
      (!isMaxLevel && selectCategoryLevel?.id !== 0)
    ) {
      return true;
    }
  ...
  return (
    <>
      <DialogContent sx={{ marginTop: `-${theme.spacing(11)}` }}>
